...
- **编号规则**：问题编号使用版本编码：翻译 V2.1.0 时发现的问题使用 `EN-210-xx`，翻译 V3.0.0 时发现的问题使用 `EN-300-xx`，翻译 V3.0.1 时发现的问题使用 `EN-301-xx`。跨文档或跨版本引用时应保留完整编号，避免不同版本的问题编号混淆。
...
当前分支为 `2.1.0-zh-CN`，中文目标为 `VDA5050_210_ZH.md`。以下条目对应本轮翻译发现的 V2.1.0 英文原文或 Markdown/表格问题；中文稿已按字段表、章节上下文和同版本协议契约规避，不修改 `VDA5050_EN.md`。

| 标记 | 编号 | 位置 | 标题 |
| --- | --- | --- | --- |
| ⚠️ | EN-210-01 | `VDA5050_EN.md:511` | `nodeSequenceId` 与 `sequenceId` 不一致 |
| ⚠️ | EN-210-02 | `VDA5050_EN.md:926` | 即时动作对象误写为 `actionStatus` |
| ⚠️ | EN-210-03 | `VDA5050_EN.md:889` | `initPosition` 完成字段路径错误 |
| ⚠️ | EN-210-04 | `VDA5050_EN.md:1351` | `state.SerialNumber` 大小写错误 |
| ⚠️ | EN-210-05 | `VDA5050_EN.md:1102` | `float 64` 类型拼写错误 |
| ⚠️ | EN-210-06 | `VDA5050_EN.md:701,743` | `actions [action]` 单复数和对象表结构问题 |
| ⚠️ | EN-210-07 | `VDA5050_EN.md:764-765` | `corridor` 宽度错误引用图 13 |
| ⚠️ | EN-210-08 | `VDA5050_EN.md:1288-1496` | factsheet 嵌套表格和版本数组行损坏 |
| ⚠️ | EN-210-09 | `VDA5050_EN.md:1320,1408,1525` | `<br/>`、`</br>` 换行标记不一致 |

### EN-210-01：`nodeSequenceId` 与 `sequenceId` 不一致

**EN 位置与问题**：`VDA5050_EN.md:511` 的订单更新说明写作 `nodeSequenceId`，但 V2.1.0 的节点/边表、后续判断和状态字段均使用 `sequenceId` 或 `lastNodeSequenceId`；文档没有定义 `nodeSequenceId`。

**中文处理**：译文按字段表和同段后文使用 `sequenceId`，不创建未定义的 `nodeSequenceId` 字段。

**官方状态**：⚠️ 待官方确认。

### EN-210-02：即时动作对象误写为 `actionStatus`

**EN 位置与问题**：`VDA5050_EN.md:926` 写成收到即时动作后向状态数组添加 `actionStatus`；`actionStatus` 实际是 `actionState` 对象内的状态字段。

**中文处理**：译文写为添加 `actionState`，并在状态表中保留字段 `actionStatus`。

**官方状态**：⚠️ 待官方确认。

### EN-210-03：`initPosition` 完成字段路径错误

**EN 位置与问题**：`VDA5050_EN.md:889` 的完成状态写作 `.agvPosition.lastNodeId = lastNodeId`，但动作参数表定义 `lastNodeId` 为顶层状态字段。

**中文处理**：译文按参数表使用顶层 `lastNodeId = lastNodeId`，不把字段嵌入 `agvPosition`。

**官方状态**：⚠️ 待官方确认。

### EN-210-04：`state.SerialNumber` 大小写错误

**EN 位置与问题**：`VDA5050_EN.md:1351` 的 factsheet 受影响参数列表写作 `state.SerialNumber`，而消息字段定义为 `state.serialNumber`。

**中文处理**：译文使用 `state.serialNumber`，字段大小写与协议字段表一致。

**官方状态**：⚠️ 待官方确认。

### EN-210-05：`float 64` 类型拼写错误

**EN 位置与问题**：`VDA5050_EN.md:1102` 将数据类型写成 `float 64`，与协议类型 `float64` 不一致。

**中文处理**：译文使用 `float64`，并保留数据类型格式。

**官方状态**：⚠️ 待官方确认。

### EN-210-06：对象表数组元素样式歧义

**EN 位置与问题**：`VDA5050_EN.md:701` 的节点对象使用字段 `actions [action]`，第 743 行的边对象却写为单数 `action [action]`；两行还同时混排对象闭合标记。`order.schema` 的节点和边字段均为复数 `actions`。

**中文处理**：译文在节点和边对象中均使用字段 `actions [action]`，保留数组元素名 `action`，并按对象层级修复闭合标记和表格列结构。

**官方状态**：⚠️ 待官方确认。

### EN-210-07：`corridor` 宽度错误引用图号

**EN 位置与问题**：`VDA5050_EN.md:764-765` 的 `leftWidth`/`rightWidth` 描述引用图 13；V2.1.0 的走廊示意图是图 10，图 13 是地图分发流程。

**中文处理**：译文将宽度字段引用改为图 10，地图分发正文仍引用图 13。

**官方状态**：⚠️ 待官方确认。

### EN-210-08：factsheet 表格嵌套和版本数组行损坏

**EN 位置与问题**：`VDA5050_EN.md:1288-1496` 包含嵌套对象表的列数/闭合标记问题，`versions[versionInfo]` 行将对象开始标记并入描述列，影响 Markdown 解析。

**中文处理**：译文保持 V2.1.0 的 `agvGeometry`、`vehicleConfig` 和 `versions[versionInfo]` 字面名称，并修复自身表格列数、嵌套层级及换行；原文问题不回写 EN。

**官方状态**：⚠️ 待官方确认。

### EN-210-09：换行标签不一致

**EN 位置与问题**：`VDA5050_EN.md:1320,1408,1525` 混用 `<br/>` 与 `</br>`，其中 `</br>` 不是有效的换行标签。

**中文处理**：译文统一使用 `<br>`，避免复制错误标签并保持表格内容可解析。

**官方状态**：⚠️ 待官方确认。

<br>

## 3.x 历史未解决的（疑似）问题条目汇总

以下 `EN-300-*`、`EN-301-*` 条目是 V3.0.0/V3.0.1 翻译工作的历史记录，不代表当前 V2.1.0 译稿的字段或待办状态。

| 标记 | 编号 | 关联问题编号 | 标题 |
| --- | --- | --- | --- |
| ❗ | EN-300-03 | EN-301-04 | 双层括号链接 |
| ❗ | EN-300-15 | EN-301-02 | Table 2 交叉引用错误 |
| ❗ | EN-300-16 | — | `actionsStates` 字段名错误 |
| ❗ | EN-300-19 | — | `string` 示例格式不一致 |
| ❗ | EN-300-23 | EN-301-05 | 序列号字段大小写及主题名单复数错误 |
| ❗ | EN-300-24 | EN-301-03 | `actions.actionsParameters` 疑点及 V3.0.1 新增冲突 |
| ❗ | EN-300-27 | — | 全文拼写、重复词及标记问题 |
| ❗ | EN-301-01 | — | `boundingBoxReference` 表重复出现 x/y 行 |
| ❗ | EN-301-02 | EN-300-15 | `actionStatus` 交叉引用错误 |
| ❗ | EN-301-03 | EN-300-24 | `versions` 数组元素名称与 Schema 不一致 |
| ❗ | EN-301-04 | EN-300-03 | `actionParameters` 链接双层括号 |
| ❗ | EN-301-05 | EN-300-23 | factsheet MQTT 参数主题名单复数不一致 |
...
译文已对发现的问题做了优化。<br>
V3.0.1 （截止 26-08-04）也修复了大部分问题：已修复 20 条、部分修复 3 条、未修复 3 条、新增冲突 1 条。
...
<br>
部分修复。<br>
V3.0.1（截止 26-08-04）第 1533 行链接已正常；第 1684 行仍为 `]((#731-format-of-action-parameters))`。<br>
在 `EN-301-04` 继续跟踪。
...
<br>
未修复。<br>
V3.0.1（截止 26-08-04）第 1269 行仍引用 Table 2；实际对应紧随其后的 Table 12。<br>
在 `EN-301-02` 继续跟踪。
...
<br>
已修复。<br>
V3.0.1（截止 26-08-04）第 1617 行章节名改为复数 `responses`，第 1633 行将 `reponses` 改为 `responses`。<br>
...
<br>
部分修复。<br>
V3.0.1（截止 26-08-04）第 2031 行已改为 `state.serialNumber`，但第 2031-2032 行仍使用单数 `response.serialNumber` 和 `response.timestamp/version/manufacturer`。<br>
在 `EN-301-05` 继续跟踪。
